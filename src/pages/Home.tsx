    ...
    <div className="min-h-screen" style={{ backgroundColor: colors.bg }}>
      ...
      <section id="catalog" className="py-12 md:py-16" style={{ backgroundColor: colors.bgAlt }}>
        <div className="max-w-7xl mx-auto px-4 lg:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-8 max-w-5xl mx-auto px-4 md:px-0">
            {products.map((product) => (
              <motion.div 
                key={product.id} 
                className="grid grid-cols-1 md:grid-cols-[200px_1fr] rounded-lg overflow-hidden card-shadow scale-[1.0] origin-center mb-0 md:mb-0 relative" 
              ...
              >
                ...
                <div className="p-8 md:p-12 flex flex-col justify-center">
                  <div className="flex items-center gap-6 mb-6">
                    <span className="w-12 h-12 rounded-full flex items-center justify-center text-white text-lg font-bold" style={{ backgroundColor: colors.accent }}>{product.id}</span>
                    <h3 className="text-3xl font-bold" style={{ color: colors.text }}>{product.name}</h3>
                  </div>
                  <p className="text-lg leading-relaxed mb-8" style={{ color: colors.textSecondary }}>{product.description}</p>
                  <button className="px-8 py-4 text-base font-semibold rounded-lg transition-all hover:scale-105 active-elevate-2 w-fit" style={{ backgroundColor: colors.button, color: colors.buttonText }}>Подробнее</button>
                </div>
              </motion.div>
            ))}
          </div>
