...
import { ChevronDown, ExternalLink, Leaf, Star, Sparkles, Heart, Shield, CircleDot, Droplets, Activity, Glasses, Ribbon } from "lucide-react";
...
const features = [
  { icon: Leaf, title: "100% натурально", desc: "Только природные компоненты" },
  { icon: Heart, title: "Забота о здоровье", desc: "Проверенные веками рецепты" },
  { icon: Shield, title: "Безопасность", desc: "Гипоаллергенные материалы" },
];
...
export default function DesignVariants() {
  ...
  return (
    <div style={{ backgroundColor: colors.bg }}>
      ...
      {/* Features strip */}
      <section style={{ backgroundColor: colors.cardBg }} className="border-y" >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {features.map((feature, idx) => (
              ...
              >
                ...
                >
                  <feature.icon className="w-7 h-7" style={{ color: colors.accent }} />
                </div>
                <div>
                  <h3 className="font-bold text-lg" style={{ color: colors.text }}>{feature.title}</h3>
